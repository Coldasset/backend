...
//Services
import { createNotification, getUserNotifications } from '../modules/notifications/notifications.services';
...
export const emitAndSaveNotification = async ({ user, type, subtype, title, message }:
  { user: string; type: 'transaction' | 'system' | 'alert'; subtype: string; title: string; message: string; }) => {
