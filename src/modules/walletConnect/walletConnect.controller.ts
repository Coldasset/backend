...
//Services
import { createWalletConnection, deleteWalletConnection, getUserWalletConnect, getWalletConnections } from './walletConnect.service';
...
export const createWalletHandler = async (request: FastifyRequest<{ Body: CreateWalletConnectInput }>, reply: FastifyReply) => {

  const decodedDetails = request.user
  const user = decodedDetails.userId;

  const userDetails = await findUserById(user);
  if (!userDetails) return sendResponse(reply, 400, false, 'User details could not be found, please try again later.');

  const existingWalletConnect = await getUserWalletConnect(user);
  if (existingWalletConnect.exists) return sendResponse(reply, 409, false, 'Multiple wallet connections are not permitted. An existing wallet is already linked to your account.');
  ...
  //Return response
  return sendResponse(reply, 201, true, 'Wallet connection successful. Please contact our support team if you require further assistance or information.', newWallet);
};

//Check if a user has connected their wallet
export const checkWalletConnectHandler = async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  return sendResponse(reply, 200, true, 'User connect wallet details was fetched successfully', hasWalletConnect);
};

//Admin Endpoint

// Fetch all wallets handlers
export const getWalletsHandler = async (request: FastifyRequest<{ Querystring: PaginationInput; }>, reply: FastifyReply) => {

  const page = Number(request.query.page || 1);
  const limit = Number(request.query.limit || 10);
  const result = await getWalletConnections(page, limit);
  return sendResponse(reply, 200, true, 'Users wallet connect was fetched successfully', result);
};

//Delete Wallet Connect
export const deleteWalletHandler = async (request: FastifyRequest<{ Params: DeleteWalletConnectInput }>, reply: FastifyReply) => {
  ...
  // Return a response
  return sendResponse(reply, 200, true, 'Wallet connection was deleted successfully.');
};
