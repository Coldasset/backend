...
const passcodeVerificationSchema = z.object({
  passcode: z.string({
    required_error: 'Passcode is required',
  })
    .min(6, 'Passcode cannot be less than size(6) numbers')
    .max(6, 'Passcode cannot be more than six(6) numbers'),
})
...
const verifyPasswordResetSchema = z.object({
  ...
  otp: z.string({
    ...
    .min(6, 'OTP cannot be less than size(6) numbers')
    .max(6, 'OTP cannot be more than six(6) numbers'),
});
...
export type PasscodeVerificationInput = z.infer<typeof passcodeVerificationSchema>;
