...
export const checkPendingTransaction = async (user: string): Promise<boolean> => {
...
export const getUserBalanceByCoin = async (userId: string) => {
  ...
  for (const tx of transactions) {
    ...
    if (tx.transactionType === TransactionType.RECEIVED) {
      ...
      balanceByCoin[coin] -= tx.amount;
    } else if (tx.transactionType === TransactionType.SWAP) {
      balanceByCoin[coin] += tx.amount;
    }
  }
...
export const getTransactions = async (transactionType?: TransactionType, page = 1, limit = 20) => {
